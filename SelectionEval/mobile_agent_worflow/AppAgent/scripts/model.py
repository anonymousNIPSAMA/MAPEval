...
class BaseModel:
    ...
    @abstractmethod
    async def get_model_response_async(self, prompt: str, images: List[str]) -> Tuple[bool, str]:
        pass
...
class QwenModel(BaseModel):
    ...
    async def get_model_response_async(self, prompt: str, images: List[str]) -> Tuple[bool, str]:
        # DashScope目前不直接支持异步调用，使用线程池执行同步调用
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, self.get_model_response, prompt, images)
